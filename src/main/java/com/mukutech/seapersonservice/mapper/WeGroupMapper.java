...
public interface WeGroupMapper extends BaseMapper<WeGroup> {
    ...
    // 统计指定用户加入的非学校社群数量
    @Select("select count(*) as group_num from we_group_member m,we_group g "
            + " where m.group_id = g.group_id  "
            + " and g.status = '1' and m.status = '1' "
            + " and g.verify_flag = '1' "
            + " and (g.school_id = 0 or g.school_id is null) "
            + " and m.uid = #{uid}")
    Integer statsJoinGroupNum(@Param("uid") Integer uid);

    // 获取指定用户加入的非学校社群的HeadImg集合
    @Select("select g.group_id,g.group_head_img from we_group_member m,we_group g "
            + " where m.group_id = g.group_id "
            + " and g.status = '1' and m.status = '1' "
            + " and g.verify_flag = '1' "
            + " and (g.school_id = 0 or g.school_id is null) "
    ...
    // 根据用户uid获取加入的学校社群列表
    @Select("select s.school_id,s.school_cnname,school_enname "
            + " from we_school s,we_group g ,we_group_member m "
            + " where s.school_id= g.school_id "
            + " and m.group_id = g.group_id "
            + " and s.status = '1' and g.status = '1' and m.status = '1' "
            + " and m.uid = #{uid}")
    List<WeGroupSchoolMyJoinVO> getJoinSchoolGroup(@Param("uid") Integer uid);

    // 根据学校Id获取该学校下的社群数量
    @Select("select count(*) as group_num "
            + " from we_school s,we_group g "
            + " where s.school_id= g.school_id "
            + " and s.status = '1' and g.status = '1' "
            + " and g.school_id = #{schoolId}")
    public Integer getSchoolGroupNum(@Param("schoolId") Integer schoolId);

    // 根据学校Id获取该学校下的社群id和头像
    @Select("select g.group_id,g.group_head_img "
            + " from we_school s,we_group g "
            + " where s.school_id= g.school_id "
            + " and s.status = '1' and g.status = '1' "
            + " and g.school_id = #{schoolId}")
    public List<WeGroupLogoVO> getSchoolGroupHeadImg(@Param("schoolId") Integer schoolId);
}
